...
final class AutoCompleteUseCaseTests: XCTestCase {
    ...
    func test_suggestions_prefix에_매칭되는_키워드를_반환한다() async {
        let keywords = [
        ...
        let result = await sut.suggestions(for: "swif")

        XCTAssertEqual(result.count, 2)
        XCTAssertTrue(result.allSatisfy { $0.keyword.lowercased().hasPrefix("swif") })
    }

    func test_suggestions_대소문자를_무시하고_매칭한다() async {
        let keywords = [
            RecentKeyword(keyword: "Swift", searchedAt: Date()),
            RecentKeyword(keyword: "SWIFT", searchedAt: Date()),
        ]
        ...
        let result = await sut.suggestions(for: "swift")
    ...
    func test_suggestions_매칭되는_항목이_없으면_빈_배열을_반환한다() async {
        let keywords = [
            RecentKeyword(keyword: "kotlin", searchedAt: Date()),
        ]
        let mockRepo = MockRecentKeywordRepository(initial: keywords)
    ...
    func test_suggestions_빈_prefix이면_빈_배열을_반환한다() async {
        let keywords = [
            RecentKeyword(keyword: "swift", searchedAt: Date()),
        ]
        let mockRepo = MockRecentKeywordRepository(initial: keywords)
        ...
        let result = await sut.suggestions(for: "")

        XCTAssertEqual(result, [])
    }

    func test_suggestions_공백만_있는_prefix이면_빈_배열을_반환한다() async {
        let keywords = [
            RecentKeyword(keyword: "swift", searchedAt: Date()),
        ]
        let mockRepo = MockRecentKeywordRepository(initial: keywords)
    ...
    func test_suggestions_maxCount를_초과하면_잘린다() async {
    ...
    func test_suggestions_maxCount가_1이면_1개만_반환한다() async {
        let keywords = [
    ...
    func test_suggestions_searchedAt_내림차순으로_정렬된다() async {
